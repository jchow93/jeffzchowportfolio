...
export default function ViewSelector({ currentView, onViewChange }: ViewSelectorProps) {
  ...
  return (
    <div className="fixed top-8 left-8 z-50" ref={dropdownRef}>
      <div className="relative">
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white/90 backdrop-blur-sm border border-gray-200 shadow-lg text-sm font-medium text-gray-700 hover:bg-white hover:shadow-xl transition-all duration-300"
        ...
        {/* Dropdown Menu */}
        {isOpen && (
          <div className="absolute top-full left-0 mt-2 w-52 rounded-lg bg-white border border-gray-200 shadow-xl overflow-hidden transition-all duration-200 ease-out">
            {VIEWS.map((view) => (
              <button
                key={view.value}
